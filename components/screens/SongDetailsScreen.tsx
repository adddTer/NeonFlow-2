...
export const SongDetailsScreen: React.FC<SongDetailsScreenProps> = ({ song, onBack, onStart, onEdit }) => {
    ...
    return (
        <div className="w-full h-full bg-[#050505] relative overflow-hidden flex flex-col animate-fade-in">
            ...
            {/* Navigation Header */}
            <div className="relative z-50 p-6 flex justify-between items-center w-full shrink-0">
                <div className="w-12"></div> {/* Spacer for balance if needed */}
                <div className="w-12"></div> {/* Spacer for balance if needed */}
            </div>

            {/* Main Content */}
            <div className="flex-1 relative z-10 flex flex-col md:flex-row items-center justify-center p-4 md:p-8 lg:p-12 gap-6 md:gap-8 lg:gap-16 overflow-y-auto custom-scrollbar w-full">
                
                {/* Left: Cover Art (Floating Card) */}
                <div className="w-full max-w-[240px] md:max-w-xs lg:max-w-md aspect-square relative group shrink-0">
                ...
                <div className="w-full max-w-xl flex flex-col">
                    ...
                    <div className="mb-6 lg:mb-8 text-center md:text-left">
                        <div className="flex flex-wrap gap-2 mb-3 lg:mb-4 justify-center md:justify-start">
                            <span className="px-2 py-1 lg:px-3 lg:py-1 rounded-full bg-white/10 border border-white/5 text-[10px] font-bold text-white uppercase tracking-wider backdrop-blur-md flex items-center gap-1.5">
                                <Zap className="w-3 h-3 text-yellow-400" />
                                {song.difficultyRating.toFixed(1)} Difficulty
                            </span>
                            <span className="px-2 py-1 lg:px-3 lg:py-1 rounded-full bg-white/10 border border-white/5 text-[10px] font-bold text-white uppercase tracking-wider backdrop-blur-md">
                                {song.laneCount} KEY
                            </span>
                        </div>
                        <h1 className="text-3xl lg:text-6xl font-black text-white leading-tight mb-2 tracking-tighter drop-shadow-2xl line-clamp-2">
                            {song.title}
                        </h1>
                        <p className="text-base lg:text-2xl text-white/60 font-medium truncate px-4 md:px-0">
                            {song.artist}
                        </p>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 lg:gap-4 mb-6 lg:mb-8">
                        <div className="bg-black/30 backdrop-blur-md rounded-xl lg:rounded-2xl p-3 lg:p-4 border border-white/5 flex flex-col justify-between hover:bg-white/5 transition-colors">
                            <Clock className="w-4 h-4 lg:w-5 lg:w-5 text-gray-500 mb-2" />
                            <div>
                                <div className="text-lg lg:text-xl font-bold text-white">{formatTime(song.duration)}</div>
                                <div className="text-[10px] text-gray-500 uppercase font-bold tracking-wider">Length</div>
                            </div>
                        </div>
                        <div className="bg-black/30 backdrop-blur-md rounded-xl lg:rounded-2xl p-3 lg:p-4 border border-white/5 flex flex-col justify-between hover:bg-white/5 transition-colors">
                            <Zap className="w-4 h-4 lg:w-5 lg:w-5 text-gray-500 mb-2" />
                            <div>
                                <div className="text-lg lg:text-xl font-bold text-white">{Math.round(song.structure.bpm)}</div>
                                <div className="text-[10px] text-gray-500 uppercase font-bold tracking-wider">BPM</div>
                            </div>
                        </div>
                        <div className="bg-black/30 backdrop-blur-md rounded-xl lg:rounded-2xl p-3 lg:p-4 border border-white/5 flex flex-col justify-between hover:bg-white/5 transition-colors">
                            <Music className="w-4 h-4 lg:w-5 lg:w-5 text-gray-500 mb-2" />
                            <div>
                                <div className="text-lg lg:text-xl font-bold text-white">{song.notes.length}</div>
                    ...
                    <div className="mt-8 flex gap-4">
                        ...
                        <button 
                            ...
                            className="flex-1 py-5 rounded-2xl bg-white text-black font-black text-xl uppercase tracking-widest hover:bg-neon-blue transition-all shadow-lg hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3 group relative overflow-hidden"
                        >
